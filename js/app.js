...
function renderLogsGrouped(data) {
   ...
   function renderFeedAnalysis(logs) {
    ...
    Object.values(logs).forEach(log => {
        ...
        // DAILY
        daily[dayKey] = (daily[dayKey] || 0) + grams;

        // WEEKLY
        weekly[weekKey] = (weekly[weekKey] || 0) + grams;
    });
    ...
    // Update total UI
    const totalEl = document.getElementById("total-feed-dispensed");
    if (totalEl) totalEl.innerText = total;

    // OPTIONAL: chart update (if you already use Chart.js)
    updateFeedChart(daily);
}
